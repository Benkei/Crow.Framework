...
namespace Crow.Graphics
{
	public sealed class SpriteFont
	{
									...
									float rotation, Vector2 origin, Vector2 scale, SpriteEffects effect, float depth)
		{
			...
			Matrix.Translation ( -origin.X, -origin.Y, 0f, out transformation );
			Matrix.Scaling ( (flippedHorz ? -scale.X : scale.X), (flippedVert ? -scale.Y : scale.Y), 1f, out temp );
			Matrix.Multiply ( ref transformation, ref temp, out transformation );
			Matrix.Translation ( flipAdjustment.X, flipAdjustment.Y, 0, out temp );
			Matrix.Multiply ( ref temp, ref transformation, out transformation );
			Matrix.RotationZ ( rotation, out temp );
			Matrix.Multiply ( ref transformation, ref temp, out transformation );
			Matrix.Translation ( position.X, position.Y, 0f, out temp );
			Matrix.Multiply ( ref transformation, ref temp, out transformation );
			...
			for ( var i = 0; i < text.Length; ++i )
			{
				...
				Vector4 destRect;
				Vector2.Transform ( ref p, ref transformation, out destRect );

				destRect.Z = currentGlyph.BoundsInTexture.Width * scale.X;
				destRect.W = currentGlyph.BoundsInTexture.Height * scale.Y;
