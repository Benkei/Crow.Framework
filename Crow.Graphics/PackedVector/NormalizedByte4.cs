...
namespace Crow.Graphics.PackedVector
{
	public struct NormalizedByte4 : IPackedVector<uint>, IEquatable<NormalizedByte4>
	{
		...
		{
			var byte4 = (((uint)Math.Round ( MathUtil.Clamp ( x, -1.0f, 1.0f ) * 127.0f )) & 0xff) << 0;
			var byte3 = (((uint)Math.Round ( MathUtil.Clamp ( y, -1.0f, 1.0f ) * 127.0f )) & 0xff) << 8;
			var byte2 = (((uint)Math.Round ( MathUtil.Clamp ( z, -1.0f, 1.0f ) * 127.0f )) & 0xff) << 16;
			var byte1 = (((uint)Math.Round ( MathUtil.Clamp ( w, -1.0f, 1.0f ) * 127.0f )) & 0xff) << 24;
