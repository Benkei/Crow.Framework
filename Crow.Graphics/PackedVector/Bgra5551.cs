...
namespace Crow.Graphics.PackedVector
{
	...
	public struct Bgra5551 : IPackedVector<UInt16>, IEquatable<Bgra5551>, IPackedVector
	{
		...
		{
			return (UInt16)(
				 (((int)Math.Round ( MathUtil.Clamp ( x, 0, 1 ) * 31.0f ) & 0x1F) << 10) |
				 (((int)Math.Round ( MathUtil.Clamp ( y, 0, 1 ) * 31.0f ) & 0x1F) << 5) |
				 (((int)Math.Round ( MathUtil.Clamp ( z, 0, 1 ) * 31.0f ) & 0x1F) << 0) |
				 ((((int)Math.Round ( MathUtil.Clamp ( w, 0, 1 ) ) & 0x1) << 15))
			);
		}
	}
}