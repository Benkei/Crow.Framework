...
namespace Crow.Graphics.PackedVector
{
	...
	public struct Bgra4444 : IPackedVector<UInt16>, IEquatable<Bgra4444>
	{
		...
		{
			return (UInt16)((((int)Math.Round ( MathUtil.Clamp ( w, 0, 1 ) * 15.0f ) & 0x0F) << 12) |
				 (((int)Math.Round ( MathUtil.Clamp ( x, 0, 1 ) * 15.0f ) & 0x0F) << 8) |
				 (((int)Math.Round ( MathUtil.Clamp ( y, 0, 1 ) * 15.0f ) & 0x0F) << 4) |
				 ((int)Math.Round ( MathUtil.Clamp ( z, 0, 1 ) * 15.0f ) & 0x0F));
		}
