...
namespace Crow.Graphics.PackedVector
{
	...
	public struct Short4 : IPackedVector<ulong>, IEquatable<Short4>
	{
		...
		private static ulong Pack(ref Vector4 vector)
		{
			...
			var word4 = ((ulong)Math.Round ( MathUtil.Clamp ( vector.X, minNeg, maxPos ) ) & 0xFFFF) << 0x00;
			var word3 = ((ulong)Math.Round ( MathUtil.Clamp ( vector.Y, minNeg, maxPos ) ) & 0xFFFF) << 0x10;
			var word2 = ((ulong)Math.Round ( MathUtil.Clamp ( vector.Z, minNeg, maxPos ) ) & 0xFFFF) << 0x20;
			var word1 = ((ulong)Math.Round ( MathUtil.Clamp ( vector.W, minNeg, maxPos ) ) & 0xFFFF) << 0x30;
