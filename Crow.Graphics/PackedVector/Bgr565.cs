...
namespace Crow.Graphics.PackedVector
{
	...
	public struct Bgr565 : IPackedVector<UInt16>, IEquatable<Bgr565>, IPackedVector
	{
		...
		{
			return (UInt16)((((int)Math.Round ( MathUtil.Clamp ( x, 0, 1 ) * 31.0f ) & 0x1F) << 11) |
				 (((int)Math.Round ( MathUtil.Clamp ( y, 0, 1 ) * 63.0f ) & 0x3F) << 5) |
				 ((int)Math.Round ( MathUtil.Clamp ( z, 0, 1 ) * 31.0f ) & 0x1F));
		}
