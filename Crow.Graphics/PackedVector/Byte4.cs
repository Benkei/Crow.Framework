...
namespace Crow.Graphics.PackedVector
{
	...
	public struct Byte4 : IPackedVector<uint>, IEquatable<Byte4>, IPackedVector
	{
		...
		private static uint Pack(ref Vector4 vector)
		{
			...
			var byte4 = (uint)Math.Round ( MathUtil.Clamp ( vector.X, min, max ) ) & 0xFF;
			var byte3 = ((uint)Math.Round ( MathUtil.Clamp ( vector.Y, min, max ) ) & 0xFF) << 0x8;
			var byte2 = ((uint)Math.Round ( MathUtil.Clamp ( vector.Z, min, max ) ) & 0xFF) << 0x10;
			var byte1 = ((uint)Math.Round ( MathUtil.Clamp ( vector.W, min, max ) ) & 0xFF) << 0x18;
