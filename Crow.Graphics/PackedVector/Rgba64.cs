...
namespace Crow.Graphics.PackedVector
{
	...
	public struct Rgba64 : IPackedVector<ulong>, IEquatable<Rgba64>, IPackedVector
	{
		...
		{
			return (ulong)(
				(((ulong)Math.Round ( MathUtil.Clamp ( x, 0, 1 ) * 65535.0f ))) |
				(((ulong)Math.Round ( MathUtil.Clamp ( y, 0, 1 ) * 65535.0f )) << 16) |
				(((ulong)Math.Round ( MathUtil.Clamp ( z, 0, 1 ) * 65535.0f )) << 32) |
					 (((ulong)Math.Round ( MathUtil.Clamp ( w, 0, 1 ) * 65535.0f )) << 48)
				);
		}
	}
}