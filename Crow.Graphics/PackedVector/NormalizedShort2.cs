...
namespace Crow.Graphics.PackedVector
{
	public struct NormalizedShort2 : IPackedVector<uint>, IEquatable<NormalizedShort2>
	{
		...
		private static uint PackInTwo(float vectorX, float vectorY)
		{
			...
			var word2 = (uint)((int)MathUtil.Clamp ( (float)Math.Round ( vectorX * maxPos ), minNeg, maxPos ) & 0xFFFF);
			var word1 = (uint)(((int)MathUtil.Clamp ( (float)Math.Round ( vectorY * maxPos ), minNeg, maxPos ) & 0xFFFF) << 0x10);
