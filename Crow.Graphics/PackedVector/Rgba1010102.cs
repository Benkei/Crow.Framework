...
namespace Crow.Graphics.PackedVector
{
	...
	public struct Rgba1010102 : IPackedVector<uint>, IEquatable<Rgba1010102>, IPackedVector
	{
		...
		{
			return (uint)(
				 (((int)Math.Round ( MathUtil.Clamp ( x, 0, 1 ) * 1023.0f ) & 0x03FF) << 0) |
				 (((int)Math.Round ( MathUtil.Clamp ( y, 0, 1 ) * 1023.0f ) & 0x03FF) << 10) |
				 (((int)Math.Round ( MathUtil.Clamp ( z, 0, 1 ) * 1023.0f ) & 0x03FF) << 20) |
				 (((int)Math.Round ( MathUtil.Clamp ( w, 0, 1 ) * 3.0f ) & 0x03) << 30)
			);
		}
	}
}