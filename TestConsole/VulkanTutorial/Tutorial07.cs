...
namespace TestConsole.VulkanTutorial
{
	...
	// ************************************************************ //
	public struct VulkanTutorial07Parameters
	{
		public RenderPass RenderPass;
		public ImageParameters Image;
		public BufferParameters UniformBuffer;
	...
	public unsafe class Tutorial07 : VulkanCommon
	{
		private VulkanTutorial07Parameters Vulkan;

		public Tutorial07()
		{
			Vulkan.RenderingResources = new RenderingResourcesData06[VulkanTutorial07Parameters.ResourcesCount];
		}
		...
		void CreateUniformBuffer()
		{
			...
			CreateBuffer (
				BufferUsageFlagBits.TransferDestinationBit | BufferUsageFlagBits.UniformBufferBit,
				 MemoryPropertyFlagBits.DeviceLocalBit,
				 ref Vulkan.UniformBuffer );
			CopyUniformBufferData ();
		}

		static float[] uniformData;

		float[] GetUniformBufferData()
		{
			float half_width = GetSwapChain.Extent.width * 0.5f;
			float half_height = GetSwapChain.Extent.height * 0.5f;

			return uniformData ?? (uniformData = Tools.GetOrthographicProjectionMatrix (
				-half_width, half_width, -half_height, half_height, -1.0f, 1.0f ));
		}

		void CopyUniformBufferData()
		{
			// Prepare data in staging buffer
			float[] uniform_data = GetUniformBufferData ();

			IntPtr staging_buffer_memory_pointer;
			vk.MapMemory ( GetDevice, Vulkan.StagingBuffer.Memory, 0, Vulkan.UniformBuffer.Size, 0, out staging_buffer_memory_pointer ).CheckError ();

			//memcpy ( staging_buffer_memory_pointer, &uniform_data[0], Vulkan.UniformBuffer.Size );
			fixed ( float* ptr = uniform_data )
			{
			...
			var flush_range = new MappedMemoryRange
			{
				...
				memory = Vulkan.StagingBuffer.Memory,      // VkDeviceMemory                         memory
				offset = 0,                                // VkDeviceSize                           offset
				size = Vulkan.UniformBuffer.Size           // VkDeviceSize                           size
			};
			vk.FlushMappedMemoryRanges ( GetDevice, 1, &flush_range );
			...
			vk.CmdCopyBuffer ( command_buffer, Vulkan.StagingBuffer.Handle, Vulkan.UniformBuffer.Handle, 1, &buffer_copy_info );
			...
			var buffer_memory_barrier = new BufferMemoryBarrier
			{
				...
				destinationAccessMask = AccessFlagBits.UniformReadBit,                         // VkAccessFlagBits                          dstAccessMask
				sourceQueueFamilyIndex = (~0U)/*VK_QUEUE_FAMILY_IGNORED*/,                            // uint32_t                               srcQueueFamilyIndex
				destinationQueueFamilyIndex = (~0U)/*VK_QUEUE_FAMILY_IGNORED*/,                            // uint32_t                               dstQueueFamilyIndex
				buffer = Vulkan.UniformBuffer.Handle,                        // VkBuffer                               buffer
				offset = 0,                                                  // VkDeviceSize                           offset
				size = (~0UL)/*VK_WHOLE_SIZE*/                                       // VkDeviceSize                           size
			};
			vk.CmdPipelineBarrier ( command_buffer,
				PipelineStageFlagBits.TransferBit,
				PipelineStageFlagBits.VertexShaderBit,
				(DependencyFlagBits)0,
				0, (MemoryBarrier*)0,
		...
		{
			var layout_bindings = stackalloc DescriptorSetLayoutBinding[2];
			layout_bindings[0] = new DescriptorSetLayoutBinding ()
			{
			...
			layout_bindings[1] = new DescriptorSetLayoutBinding ()
			{
				binding = 1,                                           // uint32_t                             binding
				descriptorType = DescriptorType.UniformBuffer,         // VkDescriptorType                     descriptorType
				descriptorCount = 1,                                   // uint32_t                             descriptorCount
				stageFlags = ShaderStageFlagBits.VertexBit,                  // VkShaderStageFlagBits                   stageFlags
				pImmutableSamplers = (Sampler*)0                       // const VkSampler                     *pImmutableSamplers
			};
			...
			var descriptor_set_layout_create_info = new DescriptorSetLayoutCreateInfo
			{
				...
				bindingCount = 2,                                      // uint32_t                             bindingCount
				pBindings = layout_bindings                            // const VkDescriptorSetLayoutBinding  *pBindings
			};

			vk.CreateDescriptorSetLayout (
				GetDevice,
				ref descriptor_set_layout_create_info,
				(AllocationCallbacks*)0,
				out Vulkan.DescriptorSet.Layout ).CheckError ();
		}

		void CreateDescriptorPool()
		{
			var pool_sizes = stackalloc DescriptorPoolSize[2];
			pool_sizes[0] = new DescriptorPoolSize
			{
				type = DescriptorType.CombinedImageSampler,   // VkDescriptorType               type
				descriptorCount = 1                           // uint32_t                       descriptorCount
			};
			pool_sizes[1] = new DescriptorPoolSize
			{
				type = DescriptorType.CombinedImageSampler,   // VkDescriptorType               type
				descriptorCount = 1                           // uint32_t                       descriptorCount
			};
			...
			var descriptor_pool_create_info = new DescriptorPoolCreateInfo
			{
				...
				pPoolSizes = pool_sizes                         // const VkDescriptorPoolSize    *pPoolSizes
			};
		...
		void UpdateDescriptorSet()
		{
			...
			var buffer_info = new DescriptorBufferInfo
			{
				buffer = Vulkan.UniformBuffer.Handle,    // VkBuffer                       buffer
				offset = 0,                              // VkDeviceSize                   offset
				range = Vulkan.UniformBuffer.Size        // VkDeviceSize                   range
			};

			var descriptor_writes = stackalloc WriteDescriptorSet[2];
			descriptor_writes[0] = new WriteDescriptorSet
			{
			...
			descriptor_writes[1] = new WriteDescriptorSet
			{
				sType = StructureType.WriteDescriptorSet,        // VkStructureType                sType
				pNext = IntPtr.Zero,                             // const void                    *pNext
				destinationSet = Vulkan.DescriptorSet.Handle,    // VkDescriptorSet                dstSet
				destinationBinding = 1,                          // uint32_t                       dstBinding
				destinationArrayElement = 0,                     // uint32_t                       dstArrayElement
				descriptorCount = 1,                             // uint32_t                       descriptorCount
				descriptorType = DescriptorType.UniformBuffer,   // VkDescriptorType               descriptorType
				pImageInfo = (DescriptorImageInfo*)0,            // const VkDescriptorImageInfo   *pImageInfo
				pBufferInfo = &buffer_info,                      // const VkDescriptorBufferInfo  *pBufferInfo
				pTexelBufferView = (BufferView*)0                // const VkBufferView            *pTexelBufferView
			};

			vk.UpdateDescriptorSets ( GetDevice, 2, descriptor_writes, 0, (CopyDescriptorSet*)0 );
		}
		...
		void CreatePipeline()
		{
			ShaderModule vertex_shader_module = CreateShaderModule ( "assets/shader07.vert.spv" );
			ShaderModule fragment_shader_module = CreateShaderModule ( "assets/shader07.frag.spv" );
		...
		{
			if ( !GetDevice.IsZero && !Vulkan.StagingBuffer.Handle.IsZero )
			{
				vk.DeviceWaitIdle ( GetDevice );
				CopyUniformBufferData ();
			}
		...
		public override void Dispose()
		{
			if ( !GetDevice.IsZero )
			{
				...
				DestroyBuffer ( ref Vulkan.UniformBuffer );
