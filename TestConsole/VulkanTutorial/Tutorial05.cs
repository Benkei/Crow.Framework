...
namespace TestConsole.VulkanTutorial
{
	...
	// ************************************************************ //
	public struct VulkanTutorial05Parameters
	{
	...
	public unsafe class Tutorial05 : VulkanCommon
	{
		...
		void CreateRenderingResources()
		{
			...
			for ( int i = 0; i < Vulkan.RenderingResources.Length; ++i )
			{
				...
				CreateSemaphore ( out Vulkan.RenderingResources[i].ImageAvailableSemaphore );
				CreateSemaphore ( out Vulkan.RenderingResources[i].FinishedRenderingSemaphore );
				CreateFence ( FenceCreateFlagBits.SignaledBit, out Vulkan.RenderingResources[i].Fence );
			}
		}
		...
		void CreateSemaphore(out Semaphore semaphore)
		{
			...
			vk.CreateSemaphore ( GetDevice, ref semaphore_create_info, (AllocationCallbacks*)0, out semaphore ).CheckError ();
		}

		void CreateFence(FenceCreateFlagBits flags, out Fence fence)
		{
			var fence_create_info = new FenceCreateInfo
			{
				sType = StructureType.FenceCreateInfo,  // VkStructureType                sType
				pNext = IntPtr.Zero,                    // const void                    *pNext
				flags = flags                           // VkFenceCreateFlagBits             flags
			};

			vk.CreateFence ( GetDevice, ref fence_create_info, (AllocationCallbacks*)0, out fence ).CheckError ();
		}
		...
		void CreateVertexBuffer()
		{
			...
			CreateBuffer ( BufferUsageFlagBits.VertexBufferBit | BufferUsageFlagBits.TransferDestinationBit, MemoryPropertyFlagBits.DeviceLocalBit, ref Vulkan.VertexBuffer );
		}
